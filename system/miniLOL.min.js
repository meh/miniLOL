...
{method:"get",asynchronous:false,onSuccess:function(c){if(!miniLOL.utils.checkXML(c.responseXML,a)){miniLOL.module.path=c.responseXML.documentElement.getAttribute("path")||"modules";var d=miniLOL.utils.fixDOM(c.responseXML).xpath("//module").filter(function(e){return e.getAttribute("name")});d.each(function(e,g){b&&miniLOL.content.set("Loading `#{name}`... [#{number}/#{total}]".interpolate({name:e.getAttribute("name"),number:g+1,total:d.length}));miniLOL.module.load(e.getAttribute("name"))||miniLOL.error(true);
...
g[2]}else{miniLOL.theme.path="themes";miniLOL.theme.name=a}if(miniLOL.theme.name==e&&miniLOL.theme.path==d)return true;g="#{path}/#{theme}".interpolate({path:miniLOL.theme.path,theme:a});Event.fire(document,":theme.load",{name:a,runtime:Boolean(b)});var l;new Ajax.Request("#{path}/theme.xml".interpolate({path:g,theme:a}),{method:"get",asynchronous:false,onSuccess:function(k){var i=miniLOL.theme.information={};k=miniLOL.utils.fixDOM(k.responseXML);i.name=k.documentElement.getAttribute("name")||"Unknown";
i.author=k.documentElement.getAttribute("author")||"Anonymous";i.homepage=k.documentElement.getAttribute("homepage")||"";miniLOL.theme.menu.node=k.documentElement.getAttribute("menu")||"menu";miniLOL.theme.content.node=k.documentElement.getAttribute("content")||"body";try{var n=k.getElementsByTagName("initialize");miniLOL.theme.initialize=n.length?new Function(n[0].firstChild.nodeValue):new Function}catch(h){l="An error occurred on the theme's initialize function:\n\n"+h.toString();return false}try{var f=
k.getElementsByTagName("finalize");miniLOL.theme.finalize=f.length?new Function(f[0].firstChild.nodeValue):new Function}catch(j){l="An error occurred on the theme's finalize function:\n\n"+j.toString();return false}i.styles=[];k.xpath("/theme/styles/style").each(function(m){i.styles.push(m.getAttribute("name"))});k.xpath("/theme/templates/*").each(function(m){miniLOL.theme.templates[m.nodeName]=Element.toObject(m)},this)},onFailure:function(){l="Could not load theme's information."}});if(l){miniLOL.error(l,
true);return false}new Ajax.Request("#{path}/template.html".interpolate({path:g,theme:a}),{method:"get",asynchronous:false,onSuccess:function(k){$(document.body).update(k.responseText)},onFailure:function(){l="Could not load template.html."}});if(l){miniLOL.error(l,true);return false}miniLOL.theme.information.styles.each(function(k){if(!miniLOL.theme.style.load(k,false,true)){miniLOL.error("Couldn't load `#{style}` style/".interpolate({style:k}),true);throw $break;}});if(miniLOL.error())return false;
...
a.removeAttribute("href");a.getAttribute("update");a.removeAttribute("update");var c=location.href.match(/^http(s)?:\/\/(.*?)([^\.]\.\w+)/)[3],d=b.match(/^http(s)?:\/\/(.*?)([^\.]\.\w+)/);d&&d[3]!=c&&'<iframe src="#{href}" #{attributes}></iframe>'.interpolate({href:b,attributes:Object.fromAttributes(a.attributes)});return""}},load:function(a,b,c){miniLOL.content.set(miniLOL.config.core.loadingMessage);Event.fire(document,":page.load",{path:a,queries:b});if(c){var d={};Object.extend(d,miniLOL.config.core);
Object.extend(d,b);document.title=(b.title||miniLOL.config.core.siteTitle).interpolate(d)}new Ajax.Request("data/#{path}?#{queries}".interpolate({path:a,queries:Object.toQueryString(b)}),{method:"get",onSuccess:function(e){b.type?miniLOL.content.set(miniLOL.resource.get("miniLOL.functions").render(b.type,e.responseText,b)):miniLOL.content.set(e.responseText);Event.fire(document,":page.loaded",e);Event.fire(document,":go",c)},onFailure:function(e){miniLOL.content.set("#{status} - #{statusText}".interpolate(e));
