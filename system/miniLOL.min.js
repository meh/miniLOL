miniLOL={version:"1.2",initialize:function(){if(miniLOL.initialized)throw Error("miniLOL has already been initialized.");miniLOL.initialized=false;miniLOL.path=location.href.match(/^(.*?)\/[^\/]*?(#|$)/)[1];miniLOL.resources={};miniLOL.tmp={};Event.observe(document,":go",miniLOL.tmp.fixScroll=function(){miniLOL.theme.content().scrollTo()});[function(){Event.observe(document,":resource.loaded",function(a){if(!(a.memo.resource.name!="miniLOL.config"||a.memo.arguments[0]!="resources/config.xml")){miniLOL.config.core||
...
b))},clearCache:function(){miniLOL.theme.template.cache={};miniLOL.theme.templates={};miniLOL.theme.templates.list={};miniLOL.theme.templates.list["default"]={global:"<div #{attributes}>#{data}</div>",before:"#{data}",after:"#{data}",link:'<div class="#{class}" id="#{id}">#{before}<a href="#{href}" target="#{target}" #{attributes}>#{text}</a>#{after}</div>',item:'<div class="#{class}" id="#{id}2>#{before}<span #{attributes}>#{text}</span>#{after}</div>',nest:'<div class="#{class}" style="#{style}">#{data}</div>',
...
d)}})}},module:{create:function(a,b){if(!b){miniLOL.error("Like, do I know how this module is done?",true);return false}b.name=a;b.root="#{path}/#{module}".interpolate({path:miniLOL.module.path,module:a});if(!b.type)b.type="active";if(!b.execute)b.execute=new Function;for(var c in b)if(Object.isFunction(b[c]))b[c]=b[c].bind(b);if(b.initialize)try{if(b.initialize()===false){if(miniLOL.error())return false;throw Error("An error occurred while initializing the module.");}}catch(d){d.fileName="#{root}/#{path}/#{module}/main.js".interpolate({root:miniLOL.path,
path:miniLOL.module.path,module:a});throw d;}miniLOL.modules[a]=b;$A(b.aliases).each(function(e){miniLOL.modules[e]=b});Event.fire(document,":module.create",b)},execute:function(a,b,c){if(!a){miniLOL.error("What module should be executed?");return false}if(!miniLOL.module.exists(a)){c&&miniLOL.error("The module `#{name}` is not loaded.".interpolate({name:a}),true);return false}if(c){miniLOL.content.set(miniLOL.config.core.loadingMessage);c={};Object.extend(c,miniLOL.config.core);Object.extend(c,b);
...
e=false;c.menu&&miniLOL.menu.change(c.menu);if(d){c.page=d[2]||d[3];if(c.page)e=miniLOL.page.get(c.page,c,a);else b||(e=miniLOL.go("#{page}&#{queries}".interpolate({page:miniLOL.config.core.homePage,queries:Object.toQueryString(c)}),true))}else if(c.module)e=miniLOL.module.execute(c.module,c,true);else if(c.page){d=c.page;delete c.page;miniLOL.page.load(d,c,a)}else e=miniLOL.go(miniLOL.config.core.homePage);e&&Event.fire(document,":go",a.getHashFragment());return e},utils:{getElementById:function(a){var b;
$A(this.getElementsByTagName("*")).each(function(c){if(c.getAttribute("id")==a){b=c;throw $break;}});return b},xpath:function(a){var b=[];if(Prototype.Browser.IE){a=this.real.selectNodes(a);for(var c=0;c<a.length;c++)b.push(a.item(c))}else{a=this.evaluate(a,this,null,XPathResult.ORDERED_NODE_SNAPSHOT_TYPE,null);for(c=0;c<a.snapshotLength;c++)b.push(a.snapshotItem(c))}return b},fixDOM:function(a){if(a){if(Prototype.Browser.IE){a={real:a};a.documentElement=a.real.documentElement;a.getElementsByTagName=
function(b){return this.real.getElementsByTagName(b)};a.getElementById=function(b){return miniLOL.utils.getElementById.call(this.real,b)};a.real.setProperty("SelectionLanguage","XPath")}else if(!Prototype.Browser.Good)a.getElementById=miniLOL.utils.getElementById;a.xpath=miniLOL.utils.xpath;return a}},checkXML:function(a,b){var c=false;a||(c="There is a syntax error.");if(a.documentElement.nodeName=="parsererror")c=a.documentElement.textContent;if(b&&c){miniLOL.error("Error while parsing #{path}\n\n#{error}".interpolate({path:b,
error:c}),true);return c}return c},getFirstText:function(a){var b="";(Object.isArray(a)?a:$A(a)).each(function(c){switch(c.nodeType){case Node.ELEMENT_NODE:throw $break;case Node.CDATA_SECTION_NODE:case Node.TEXT_NODE:if(!c.nodeValue.isEmpty()){b=c.nodeValue.strip();throw $break;}}});return b},exists:function(a){var b=false;new Ajax.Request(a,{method:"head",asynchronous:false,onSuccess:function(){b=true}});return b},includeCSS:function(a){var b;if(b=miniLOL.theme.style.list[a])return b;else if(b=
$$("link").find(function(c){return c.getAttribute("href")==a}))return miniLOL.theme.style.list[a]=b;else if(b=miniLOL.utils.exists(a)){b=new Element("link",{rel:"stylesheet",href:a,type:"text/css"});$$("head")[0].insert(b);Event.fire(document,":css.include",a);return b}else return false},css:function(a,b){var c=(new Element("style",{type:"text/css"})).update(a);b&&c.setAttribute("id",b);$$("head").first().appendChild(c);Event.fire(document,":css.create",c);return c},execute:function(a){var b,c;new Ajax.Request(a,
{method:"get",asynchronous:false,evalJS:false,onSuccess:function(d){try{b=window.eval(d.responseText)}catch(e){c=e;c.fileName=a;c.lineNumber-=5}},onFailure:function(d){c=Error("Failed to retrieve `#{file}` (#{status} - #{statusText}).".interpolate({file:a,status:d.status,statusText:d.statusText}));c.fileName=a;c.lineNumber=0}});if(c)throw c;return b},include:function(a){var b=false;new Ajax.Request(a,{method:"get",asynchronous:false,evalJS:false,onSuccess:function(c){try{window.eval(c.responseText);
...
miniLOL.Storage.Backends={available:function(){try{return window.localStorage?miniLOL.Storage.Backends.LocalStorage:window.globalStorage?miniLOL.Storage.Backends.GlobalStorage:document.createElment("link").addBehavior?miniLOL.Storage.Backends.UserDataBehavior:miniLOL.Storage.Backends.Cookie}catch(a){return miniLOL.Storage.Backends.Null}},LocalStorage:Class.create(miniLOL.Storage.Backend,{initialize:function($super,b){$super(b);this.data=miniLOL.Storage.unserialize(window.localStorage["__miniLOL_storage_"+
b])||{}},save:function(){var a=miniLOL.Storage.serialize(this.data);this.size=a.length;window.localStorage["__miniLOL_storage_"+this.name]=a}}),GlobalStorage:Class.create(miniLOL.Storage.Backend,{initialize:function($super,b){$super(b);this.data=miniLOL.Storage.unserialize(window.globalStorage[window.location.hostname]["__miniLOL_storage_"+b])||{}},save:function(){var a=miniLOL.Storage.serialize(this.data);this.size=a.length;window.globalStorage[window.location.hostname]["__miniLOL_storage_"+this.name]=
a}}),UserDataBehavior:Class.create(miniLOL.Storage.Backend,{initialize:function($super,b){$super(b);this.element=document.createElement("link");this.element.addBehavior("#default#userData");$$("head")[0].appendChild(this.element);this.element.load("__miniLOL_storage_"+b);var c=this.element.getAttribute("__miniLOL_storage_"+b);this.data=miniLOL.Storage.unserialize(c);this.size=c.length},save:function(){var a=miniLOL.Storage.serialize(this.data);this.size=a.length;this.element.setAttribute("__miniLOL_storage_"+
this.name,a);this.element.save("__miniLOL_storage_"+this.name)}}),Cookie:Class.create(miniLOL.Storage.Backend,{initialize:function($super,b){$super(b);this.jar=new CookieJar({expires:31536E3});var c=this.jar.get("__miniLOL_storage_"+b);this.data=miniLOL.Storage.unserialize(c);this.size=c.length},save:function(){var a=miniLOL.Storage.serialize(this.data);this.size=a.length;this.jar.set("__miniLOL_storage_"+this.name,a)}}),Null:Class.create(miniLOL.Storage.Backend,{save:Prototype.emptyFunction})};
