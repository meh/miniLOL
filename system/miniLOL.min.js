...
b})}else{b=this.calls;this.calls=[]}return b},data:function(){return this.wrapper.data}});miniLOL.Storage=Class.create({});unFocus.History.addEventListener("historyChange",function(a){a?miniLOL.go(a):miniLOL.go(miniLOL.config.core.homePage)});if(navigator.userAgent.match(/Chrome/))Prototype.Browser.Chrome=true;if(navigator.userAgent.match(/Safari/)&&!Prototype.Browser.Chrome)Prototype.Browser.Safari=true;
...
"<br/>")})};else if(Prototype.Browser.Chrome||Prototype.Browser.Safari)Error.prototype.toString=function(){return"#{name}: #{message}<br/><br/>#{stack}".interpolate({name:this.name,message:this.message,stack:this.stack.replace(/\n/g,"<br/>")})};Function.prototype.clone=Prototype.Browser.IE?function(){var a=this.toString();return new Function(a.substring(a.indexOf("{")+1,a.lastIndexOf("}")))}:function(){return eval("("+this.toString().replace(/^function .*?\(/,"function (")+")")};miniLOL.storage=$.jStorage;
Object.extend(Object,{isBoolean:function(a){return a.constructor===Boolean},fromAttributes:function(a){for(var b={},c=0;c<a.length;c++)b[a.item(c).nodeName]=a.item(c).nodeName;return b},toQueryString:function(a){var b="",c;for(c in a)b+="#{name}=#{value}&".interpolate({name:c,value:a[c]});return b.substr(0,b.length-1)}});
...
if(!Object.isFunction(Object.create))Object.create=function(a,b){var c=Object(a);Object.defineProperties(c,b);return c};Object.extend(String,{fromAttributes:function(a){for(var b="",c=0;c<a.length;c++)b+='#{name}="#{value}" '.interpolate({name:a.item(c).nodeName,value:a.item(c).nodeValue});return b}});
...
getHashFragment:function(){var a=this.match(/(#.*)$/);return a?a[1]:""}});
