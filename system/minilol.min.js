...
module:b});miniLOL.error("An error occurred while executing the module `#{name}`\n\n#{file} @ #{line}:\n#{error}".interpolate({name:b,file:f.fileName,line:f.lineNumber,error:f.toString()}),miniLOL.theme.content());return false}Event.fire(document,":module.executed",{name:b,arguments:a,result:d});if(Object.isUndefined(d))d=true;return d},load:function(b){Event.fire(document,":module.load",b);try{miniLOL.utils.require("#{path}/#{module}/main.js".interpolate({path:miniLOL.module.path,module:b}));if(miniLOL.error())return false;
...
f;c.fileName=b;c.lineNumber-=5}},onFailure:function(d){c=new Error("Failed to retrieve the file (#{status} - #{statusText}).".interpolate(d));c.fileName=b;c.lineNumber=0}});if(c)throw c;return a}}};
