...
after:miniLOL.theme.template.list[h].after.interpolate({data:k}),data:miniLOL.page.parse(toParse,[b])})}}else if(g.nodeType==Node.CDATA_SECTION_NODE||g.nodeType==Node.TEXT_NODE)if(g.nodeValue.replace(/[\s\n]+/g,""))n+=miniLOL.theme.template.list[h].data.interpolate({before:miniLOL.theme.template.list[h].before.interpolate({data:q}),after:miniLOL.theme.template.list[h].after.interpolate({data:k}),data:g.nodeValue})});return miniLOL.theme.template.list[h].global.interpolate(Object.extend(Object.fromAttributes(list.attributes),
{attributes:String.fromAttributes(list.attributes),data:n}))},include:function(b){b=b.cloneNode(false);var a=b.getAttribute("href");b.removeAttribute("href");b.getAttribute("update");b.removeAttribute("update");var c=location.href.match(/^http(s)?:\/\/(.*?)([^\.]\.\w+)/)[3],d=a.match(/^http(s)?:\/\/(.*?)([^\.]\.\w+)/);d&&d[3]!=c&&'<iframe src="#{href}" #{attributes}></iframe>'.interpolate({href:a,attributes:Object.fromAttributes(b.attributes)});return""}},load:function(b,a,c){miniLOL.content.set(miniLOL.config.core.loadingMessage);
