...
miniLOL.config={}},parse:function(b,a){if(a){var c="";$A(b.childNodes).each(function(d){if(!(d.nodeType!=Node.CDATA_SECTION_NODE&&d.nodeType!=Node.TEXT_NODE)){d=d.nodeValue;d.match(/^[\s\n]*$/m)||(c+=d)}});return c}if(b.nodeType!=Node.ELEMENT_NODE)return null;c={};$A(b.childNodes).each(function(d){if(d.nodeType==Node.ELEMENT_NODE)c[d.nodeName]=d.getElementsByTagName("*").length==0?this.parse(d,true):this.parse(d)});return c}});miniLOL.resources.config.load("resources/config.xml");$(document.body).update(miniLOL.config.core.loadingMessage)},
...
nest:'<div class="#{class}" style="#{style}">#{data}</div>',data:'<div class="data">#{before}#{data}#{after}</div>'}}},load:function(b,a,c){miniLOL.theme.unload();var d=miniLOL.theme.path,e=miniLOL.theme.name,j=b.match(/^(.+?):(.+)$/);if(j){miniLOL.theme.path=j[1];miniLOL.theme.name=j[2]}else{miniLOL.theme.path="themes";miniLOL.theme.name=b}if(miniLOL.theme.name==e&&miniLOL.theme.path==d)return true;j="#{path}/#{theme}".interpolate({path:miniLOL.theme.path,theme:b});Event.fire(document,":theme.load",
{name:b,runtime:Boolean(a)});var o;new Ajax.Request("#{path}/theme.xml".interpolate({path:j,theme:b}),{method:"get",asynchronous:false,onSuccess:function(h){var n=miniLOL.theme.informations={};h=miniLOL.utils.fixDOM(h.responseXML);n.name=h.documentElement.getAttribute("name")||"Unknown";n.author=h.documentElement.getAttribute("author")||"Anonymous";n.homepage=h.documentElement.getAttribute("homepage")||"";miniLOL.theme.menu._node=h.documentElement.getAttribute("menu")||"menu";miniLOL.theme.content._node=
h.documentElement.getAttribute("content")||"body";try{var g=h.getElementsByTagName("initialize");miniLOL.theme.initialize=g.length?new Function(g[0].firstChild.nodeValue):new Function}catch(f){o="An error occurred on the theme's initialize function:\n\n"+f.toString();return false}try{var i=h.getElementsByTagName("finalize");miniLOL.theme.finalize=i.length?new Function(i[0].firstChild.nodeValue):new Function}catch(p){o="An error occurred on the theme's finalize function:\n\n"+p.toString();return false}n.styles=
[];$A(h.getElementsByTagName("style")).each(function(k){n.styles.push(k.getAttribute("name"))});g=h.getElementsByTagName("templates");var q;if(g&&(q=g.getElementsByTagName("list")))$A(q.getElementsByTagName("template")).each(function(k){var l,m=k.getAttribute("name")||"default";miniLOL.theme.template.list[m]={};if((l=k.getElementsByTagName("global")).length)miniLOL.theme.template.list[m].global=l[0].firstChild.nodeValue;if((l=k.getElementsByTagName("before")).length)miniLOL.theme.template.list[m].before=
...
l[0].firstChild.nodeValue})},onFailure:function(){o="Could not load theme's informations."}});if(o){miniLOL.error(o);return false}new Ajax.Request("#{path}/template.html".interpolate({path:j,theme:b}),{method:"get",asynchronous:false,onSuccess:function(h){$(document.body).update(h.responseText)},onFailure:function(){o="Could not load template.html."}});if(o){miniLOL.error(o);return false}for(d=0;d<miniLOL.theme.informations.styles.length;d++)if(!miniLOL.theme.style.load(miniLOL.theme.informations.styles[d],
false,true)){miniLOL.error("Couldn't load `#{style}` style.".interpolate({style:miniLOL.theme.informations.styles[d]}));return false}if(a&&miniLOL.initialized){miniLOL.menu.change(miniLOL.menu.current,true);miniLOL.go(location.href)}miniLOL.theme.data={};!c&&miniLOL.theme.initialize&&miniLOL.theme.initialize();Event.fire(document,":theme.loaded",{name:b,runtime:Boolean(a)});return true},unload:function(b){miniLOL.theme.template.clearCache();if(miniLOL.theme.name){Event.fire(document,":theme.unload",
{name:miniLOL.theme.name});!b&&miniLOL.theme.finalize&&miniLOL.theme.finalize();delete miniLOL.theme.data;$A(miniLOL.theme.informations.styles).each(function(a){miniLOL.theme.style.unload(a)});delete miniLOL.theme.name;delete miniLOL.theme.initialize;delete miniLOL.theme.finalize;delete miniLOL.theme.informations}},content:function(){return $(miniLOL.theme.content._node)},menu:function(){return $(miniLOL.theme.menu._node)},deprecated:function(){miniLOL.theme.path="themes";miniLOL.theme.content._node=
...
exists:function(b){return Boolean(miniLOL.menus[b])},parse:function(b,a){a=a||0;var c=miniLOL.theme.template.menu();if(!c||!b)if(miniLOL.error())return false;var d=true,e="";$A(b.childNodes).each(function(j){switch(j.nodeType){case Node.ELEMENT_NODE:e+=j.nodeName=="menu"?miniLOL.menu.parsers.layer(c,a).menu.interpolate({data:miniLOL.menu.parse(j,a)}):j.nodeName=="item"?miniLOL.menu.parsers.item(j,c,a):miniLOL.menu.parsers.other(j,c);break;case Node.CDATA_SECTION_NODE:case Node.TEXT_NODE:d||(e+=j.nodeValue);
d=false;break}});return e.replace(/[\s\n]*/g,"")?a==0?miniLOL.menu.parsers.layer(c,a).menu.interpolate({data:e}):e:""},parsers:{layer:function(b,a){var c={menu:"",item:""};if(b){var d=b.getElementById(a)||b.getElementById("default");if(d){if(d.getElementsByTagName("menu").length)c.menu=d.getElementsByTagName("menu")[0].firstChild.nodeValue;if(d.getElementsByTagName("item").length)c.item=d.getElementsByTagName("item")[0].firstChild.nodeValue;if(!c.menu)c.menu="#{data}";if(!c.item)c.item='<a href="#{href}" #{attributes}>#{text}</a> '}}return c},
item:function(b,a,c){item=b.cloneNode(true);var d=item.getAttribute("class")||"";item.removeAttribute("class");var e=item.getAttribute("id")||"";item.removeAttribute("id");var j=item.getAttribute("src")||item.getAttribute("href")||item.getAttribute("url")||"";item.removeAttribute("src");item.removeAttribute("href");item.removeAttribute("url");return miniLOL.menu.parsers.layer(a,c).item.interpolate(Object.extend(Object.fromAttributes(item.attributes),{"class":d,id:e,url:j,src:j,href:j,attributes:String.fromAttributes(item.attributes),
text:miniLOL.utils.getFirstText(b.childNodes),data:miniLOL.menu.parse(b,c+1)}))},other:function(b,a){var c={};if(!b||!a)return"";a=a.getElementsByTagName(b.nodeName);if(a.length==0)return"";else a=a[0];var d=miniLOL.utils.getFirstText(a.childNodes);$A(b.childNodes).each(function(e){if(e.nodeType==Node.ELEMENT_NODE)c[e.nodeName]=miniLOL.menu.parsers.other(e,a)});c.text=miniLOL.utils.getFirstText(b.childNodes);return d.interpolate(Object.extend(c,Object.fromAttributes(b.attributes)))}}},page:{get:function(b,
a,c){miniLOL.content.set(miniLOL.config.core.loadingMessage);Event.fire(document,":page.get",{name:b,queries:a});var d=miniLOL.pages.data[b],e=a.type;if(!d){miniLOL.content.set("404 - Not Found");return false}if(d.getAttribute("alias")){typeof a[b]!="string"&&delete a[b];delete a.page;if(!a.title&&d.getAttribute("title"))a.title=encodeURIComponent(d.getAttribute("title"));if(a=Object.toQuery(a))a="&"+a;d=d.getAttribute("alias");if(!d.isURL()&&d.charAt(0)!="#")d="#"+d;return miniLOL.go(d+a)}if(e==
null)e=d.getAttribute("type");if(c){c={};Object.extend(c,miniLOL.config.core);Object.extend(c,a);document.title=(a.title||d.getAttribute("title")||miniLOL.config.core.siteTitle).interpolate(c)}if(miniLOL.pages.cache[b]){miniLOL.functions[e]?miniLOL.content.set(miniLOL.functions[e](miniLOL.pages.cache[b],a)):miniLOL.content.set(miniLOL.pages.cache[b]);return true}if(c=d.getAttribute("arguments")){c=c.replace(/[ ,]+/g,"&amp;").parseQuery();for(var j in c)if(a[j]==null)a[j]=c[j]}d=miniLOL.page.parse(d);
miniLOL.pages.cache[b]=d;if(miniLOL.functions[e])d=miniLOL.functions[e](d,a);miniLOL.content.set(d);return true},parse:function(b,a){var c="";$A(b.childNodes).each(function(d){switch(d.nodeType){case Node.ELEMENT_NODE:if(Object.isFunction(miniLOL.page.parsers[d.nodeName]))c+=miniLOL.page.parsers[d.nodeName](d,a);break;case Node.CDATA_SECTION_NODE:case Node.TEXT_NODE:c+=d.nodeValue;break}});return c},parsers:{list:function(b,a){list=b.cloneNode(false);a=a||[b];var c=list.getAttribute("before")||a[0].getAttribute("before")||
"";list.removeAttribute("before");var d=list.getAttribute("after")||a[0].getAttribute("after")||"";list.removeAttribute("after");var e=list.getAttribute("arguments")||a[0].getAttribute("arguments")||"";list.removeAttribute("arguments");var j=list.getAttribute("type")||a[0].getAttribute("type")||"";list.removeAttribute("type");var o=list.getAttribute("menu")||a[0].getAttribute("menu");list.removeAttribute("menu");var h=list.getAttribute("template");list.removeAttribute("template");miniLOL.theme.template.list[h]||
(h="default");var n="";$A(b.childNodes).each(function(g){if(g.nodeType==Node.ELEMENT_NODE)if(g.nodeName=="link"){var f=g.cloneNode(true),i=f.getAttribute("src")||f.getAttribute("href")||f.getAttribute("url")||"";f.removeAttribute("src");f.removeAttribute("href");f.removeAttribute("url");var p=f.getAttribute("target");f.removeAttribute("target");g=f.getAttribute("text")||"";f.removeAttribute("text");var q=f.getAttribute("before")||c||"";f.removeAttribute("before");var k=f.getAttribute("after")||d||
"";f.removeAttribute("after");var l=f.getAttribute("domain")||"";f.removeAttribute("domain");var m=f.getAttribute("arguments")||e;f.removeAttribute("arguments");var t=f.getAttribute("menu")||o;f.removeAttribute("menu");var r=f.getAttribute("title")||"";f.removeAttribute("title");var s=i.isURL(),u=f.getAttribute("class")||"";f.removeAttribute("class");var v=f.getAttribute("id")||"";f.removeAttribute("id");if(p||s){i=!s?"data/"+i:i;p=p||"_blank";g=g||i}else{s=f.getAttribute("type")||j||"";f.removeAttribute("type");
i=l=="in"||i.charAt(0)=="#"?i.charAt(0)=="#"?i:"#"+i:"#page="+i;g=g||i;m=m?"&"+m.replace(/[ ,]+/g,"&amp;"):"";s=s?"&type="+s:"";t=miniLOL.menu.enabled()&&t?"&amp;menu="+t:"";p="";if(r)r=r.interpolate({text:g,url:i,href:i,src:i});r=r?"&title="+encodeURIComponent(r):"";i=i+m+s+t+r}n+=miniLOL.theme.template.list[h].link.interpolate(Object.extend(Object.fromAttributes(f.attributes),{"class":u,id:v,attributes:String.fromAttributes(f.attributes),before:miniLOL.theme.template.list[h].before.interpolate({data:q}),
after:miniLOL.theme.template.list[h].after.interpolate({data:k}),url:i,src:i,href:i,target:p,text:g,title:r}))}else if(g.nodeName=="item"){f=g.cloneNode(true);g=f.getAttribute("text")||"";f.removeAttribute("text");q=f.getAttribute("before")||c||"";f.removeAttribute("before");k=f.getAttribute("after")||d||"";f.removeAttribute("after");i=f.getAttribute("class")||"";f.removeAttribute("class");p=f.getAttribute("id")||"";f.removeAttribute("id");n+=miniLOL.theme.template.list[h].item.interpolate(Object.extend(Object.fromAttributes(f.attributes),
{"class":i,id:p,attributes:String.fromAttributes(f.attributes),before:miniLOL.theme.template.list[h].before.interpolate({data:q}),after:miniLOL.theme.template.list[h].after.interpolate({data:k}),text:g}))}else if(g.nodeName=="list")n+=miniLOL.page.parse({childNodes:[g]},[b]);else{if(g.nodeName=="nest"){toParse=g.cloneNode(true);n+=miniLOL.theme.template.list[h].nest.interpolate({"class":g.getAttribute("class")||"",style:g.getAttribute("style")||"",before:miniLOL.theme.template.list[h].before.interpolate({data:q}),
after:miniLOL.theme.template.list[h].after.interpolate({data:k}),data:miniLOL.page.parse(toParse,[b])})}}else if(g.nodeType==Node.CDATA_SECTION_NODE||g.nodeType==Node.TEXT_NODE)if(g.nodeValue.replace(/[\s\n]+/g,""))n+=miniLOL.theme.template.list[h].data.interpolate({before:miniLOL.theme.template.list[h].before.interpolate({data:q}),after:miniLOL.theme.template.list[h].after.interpolate({data:k}),data:g.nodeValue})});return miniLOL.theme.template.list[h].global.interpolate(Object.extend(Object.fromAttributes(b.attributes),
{attributes:String.fromAttributes(b.attributes),data:n}))},include:function(b){b=b.cloneNode(false);var a=b.getAttribute("href");b.removeAttribute("href");b.getAttribute("update");b.removeAttribute("update");var c=location.href.match(/^http(s)?:\/\/(.*?)([^\.]\.\w+)/)[3],d=a.match(/^http(s)?:\/\/(.*?)([^\.]\.\w+)/);d&&d[3]!=c&&'<iframe src="#{href}" #{attributes}></iframe>'.interpolate({href:a,attributes:Object.fromAttributes(b.attributes)});return""}},load:function(b,a,c){miniLOL.content.set(miniLOL.config.core.loadingMessage);
Event.fire(document,":page.load",{path:b,queries:a});if(c){var d={};Object.extend(d,miniLOL.config.core);Object.extend(d,a);document.title=(a.title||miniLOL.config.core.siteTitle).interpolate(d)}new Ajax.Request("data/#{path}?#{queries}".interpolate({path:b,queries:Object.toQuery(a)}),{method:"get",onSuccess:function(e){miniLOL.functions[a.type]?miniLOL.content.set(miniLOL.functions[a.type](e.responseText,a)):miniLOL.content.set(e.responseText);Event.fire(document,":page.loaded",e);Event.fire(document,
":go",c)},onFailure:function(e){miniLOL.content.set("#{status} - #{statusText}".interpolate(e));Event.fire(document,":page.loaded",e)}})}},module:{create:function(b,a){if(!a){miniLOL.error("Like, do I know how this module is done?");return false}a.name=b;a.root="#{path}/#{module}".interpolate({path:miniLOL.module.path,module:b});if(!a.type)a.type="active";if(!a.execute)a.execute=new Function;for(var c in a)if(Object.isFunction(a[c]))a[c]=a[c].bind(a);if(a.initialize)try{if(a.initialize()===false){if(miniLOL.error())return false;
throw new Error("An error occurred while initializing the module.");}}catch(d){d.fileName="#{root}/#{path}/#{module}/main.js".interpolate({root:miniLOL.path,path:miniLOL.module.path,module:b});throw d;}miniLOL.modules[b]=a;$A(a.aliases).each(function(e){miniLOL.modules[e]=a});Event.fire(document,":module.create",a)},execute:function(b,a,c){if(!b){miniLOL.error("What module should be executed?");return false}if(!miniLOL.module.exists(b)){c&&miniLOL.error("The module `#{name}` isn't loaded.".interpolate({name:b}),
miniLOL.theme.content(),true);return false}if(c){miniLOL.content.set(miniLOL.config.core.loadingMessage);c={};Object.extend(c,miniLOL.config.core);Object.extend(c,a);document.title=(a.title||miniLOL.module.get(b).title||miniLOL.config.core.siteTitle).interpolate(c)}a=Object.isArray(a)?a:[a];Event.fire(document,":module.execute",{name:b,arguments:a});var d;try{d=miniLOL.module.get(b).execute.apply(miniLOL.module.get(b),a)}catch(e){e.fileName="#{root}/#{path}/#{module}/main.js".interpolate({root:miniLOL.path,
path:miniLOL.module.path,module:b});miniLOL.error("An error occurred while executing the module `#{name}`\n\n#{file} @ #{line}:\n#{error}".interpolate({name:b,file:e.fileName,line:e.lineNumber,error:e.toString()}),miniLOL.theme.content());return false}Event.fire(document,":module.executed",{name:b,arguments:a,result:d});if(Object.isUndefined(d))d=true;return d},load:function(b){Event.fire(document,":module.load",b);try{miniLOL.utils.require("#{path}/#{module}/main.js".interpolate({path:miniLOL.module.path,
module:b}));if(miniLOL.error())return false;if(!miniLOL.module.exists(b))throw new Error("Something went wrong while loading the module `#{name}`.".interpolate({name:b}));Event.fire(document,":module.loaded",b);return true}catch(a){miniLOL.error("An error occurred while loading the module `#{name}`\n\n#{root}/#{file} @ #{line}:\n\n#{error}".interpolate({name:b,root:miniLOL.path,file:a.fileName,line:a.lineNumber,error:a.toString()}),miniLOL.theme.content());return false}},get:function(b){return miniLOL.modules[b]},
exists:function(b){return Boolean(miniLOL.module.get(b))},dependencies:{check:function(){for(var b in miniLOL.modules){var a=miniLOL.module.get(b).dependencies;if(a)for(var c=0;c<a.length;c++)if(!miniLOL.module.get(a[c]))throw{module:b,require:a[c]};}return true},needs:function(b,a,c,d){miniLOL.module.get(b)?a.call(c||window):setTimeout(function(){miniLOL.module.dependencies.needs(b,a,c)},d||10)}}},go:function(b){if(!b.startsWith(miniLOL.path)&&b.isURL()){location.href=b;return true}var a=b.parseQuery(),
c=/#(([^=&]*)&|([^=&]*)$)/.exec(b),d=false;a.menu&&miniLOL.menu.change(a.menu);if(c){a.page=c[2]||c[1];d=a.page&&a.page!="&"?miniLOL.page.get(a.page,a,b):miniLOL.go("#{page}&#{queries}".interpolate({page:miniLOL.config.core.homePage,queries:Object.toQuery(a)}))}else if(a.page){c=a.page;delete a.page;miniLOL.page.load(c,a,b)}else d=a.module?miniLOL.module.execute(a.module,a,true):miniLOL.go(miniLOL.config.core.homePage);d&&Event.fire(document,":go",b);return d},utils:{getElementById:function(b){for(var a=
this.getElementsByTagName("*"),c=0;c<a.length;c++)if(a[c].getAttribute("id")==b)return a[c]},fixDOM:function(b){if(!b||Prototype.Browser.Good)return b;if(Prototype.Browser.IE){var a=b;b={};b._real=a;b.documentElement=a.documentElement;b.getElementsByTagName=function(c){return this._real.getElementsByTagName(c)};b.getElementById=function(c){return miniLOL.utils.getElementById.call(this._real,c)}}else b.getElementById=miniLOL.utils.getElementById;return b},checkXML:function(b,a){var c=false;b||(c="There's a syntax error.");
if(b.documentElement.nodeName=="parsererror")c=b.documentElement.textContent;if(a&&c){miniLOL.error("Error while parsing #{path}\n\n#{error}".interpolate({path:a,error:c}));return c}return c},getFirstText:function(b){for(var a=0;a<b.length;a++)switch(b[a].nodeType){case Node.ELEMENT_NODE:return"";case Node.CDATA_SECTION_NODE:case Node.TEXT_NODE:if(b[a].nodeValue.replace(/[\s\n]/g,""))return b[a].nodeValue;break}return""},fileExists:function(b){var a=false;new Ajax.Request(b,{method:"head",asynchronous:false,
onSuccess:function(){a=true}});return a},includeCSS:function(b){var a=miniLOL.utils.fileExists(b);if(a){a=new Element("link",{rel:"stylesheet",href:b,type:"text/css"});$$("head")[0].insert(a)}return a},include:function(b){var a=null;new Ajax.Request(b,{method:"get",asynchronous:false,evalJS:false,onSuccess:function(c){try{a=window.eval(c.responseText)}catch(d){}}});return a},require:function(b){var a,c;new Ajax.Request(b,{method:"get",asynchronous:false,evalJS:false,onSuccess:function(d){try{a=window.eval(d.responseText)}catch(e){c=
