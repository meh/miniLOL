...
b.apply=function(a){this.filters.each(function(c){a=c.apply(a)});return a};b.load=function(a){new Ajax.Request(a,{method:"get",asynchronous:false,onSuccess:function(c){var d=miniLOL.utils.XML.check(c.responseXML);if(d)miniLOL.error("Error while parsing `#{filter}`\n\n#{error}".interpolate({filter:a,error:d}));else{var e=miniLOL.utils.XML.fix(c.responseXML);e.xpath("/filters/filter").each(function(f){b.filters.push(new b.Filter(f,e.documentElement.getAttribute("censor")))});b.paths.push(a)}}})};b.reload=
